                            ...
                            <div class="order-status-amount">
                                <c:choose>
                                    <c:when test="${not empty order.status}">
                                        ...
                                        <c:choose>
                                            <c:when test="${fn:containsIgnoreCase(order.status.name, 'Pending') or fn:containsIgnoreCase(order.status, 'chờ')}">
                                                <c:set var="statusClass" value="status-pending" />
                                            </c:when>
                                            <c:when test="${fn:containsIgnoreCase(order.status.name, 'Processing') or fn:containsIgnoreCase(order.status, 'xử lý')}">
                                                <c:set var="statusClass" value="status-processing" />
                                            </c:when>
                                            <c:when test="${fn:containsIgnoreCase(order.status.name, 'Shipped') or fn:containsIgnoreCase(order.status, 'giao')}">
                                                <c:set var="statusClass" value="status-shipped" />
                                            </c:when>
                                            <c:when test="${fn:containsIgnoreCase(order.status.name, 'Delivered') or fn:containsIgnoreCase(order.status, 'hoàn thành')}">
                                                <c:set var="statusClass" value="status-delivered" />
                                            </c:when>
                                            <c:when test="${fn:containsIgnoreCase(order.status.name, 'Cancelled') or fn:containsIgnoreCase(order.status, 'hủy')}">
                                    ...
                                    <c:when test="${not empty order.orderItems}">
                                        ...
                                        <c:forEach var="item" items="${order.orderItems}" end="1">
                                            <div class="product-preview">
                                                <img src="${not empty item.product.imageUrl ? item.product.imageUrl : 'https://via.placeholder.com/60x60?text=No+Image'}"
                                ...
                                <div class="summary-info">
                                    ...
                                    <div class="summary-item">
                                        <i class="fas fa-credit-card"></i>
                                            ${not empty order.paymentMethod.name ? order.paymentMethod.name : 'Chưa xác định'}
                                    </div>
